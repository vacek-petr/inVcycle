...
vcycleAbsoluteTolerance = 0 ;
vcycleMaximumNumberOfIterations = 15;
...
% fun = @(x) x - vcycle(A,P,numberOfLevels,A{numberOfLevels}*x,zeros(size(F)),smoother,Solver("backslash"));
% errorPropagationMatrixAnorm  = eigs(fun,size(F,1),1,'largestabs',Tolerance=1e-4);

errorPropagationMatrixAnorm = 0.1466; % Poisson
% errorPropagationMatrixAnorm = 0.6177; % jump-1024

theta = 1e-11; % or 1e-4;
listOfEpsilons = (1-errorPropagationMatrixAnorm)*theta;
