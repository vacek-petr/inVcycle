classdef MatrixHierarchy< handle
    ...
    methods
        ...
        function switchToMp(obj,numberOfDigits)
            mp.Digits(numberOfDigits);
            for j=1:obj.numberOfLevels
                obj.A{j} = mp(obj.A{j});
                obj.P{j} = mp(obj.P{j});
                obj.F{j} = mp(obj.F{j});
            end
            obj.approxBackslash{obj.numberOfLevels} = obj.A{j}\obj.F{j};              
        end
    end
end