...
function update(obj,mh,approx,iter,timeVcycle,coarsestLevelSolverInfo,options)
    ...
    if(iter <= size(obj.approxVcycleBackslash,2))
        obj.diffApproxApproxVcycleBackslashAnorm(iter) = sqrt((approx - obj.approxVcycleBackslash(:,iter))'*A{J}*(approx - obj.approxVcycleBackslash(:,iter)));
    end
